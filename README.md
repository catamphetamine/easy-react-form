...
Is simply a shorthand for the non-default `Form` export
...
Where `'orderForm'` argument is the explicitly set application-wide globally unique `id` of the form (inside Redux state tree) whereas in the former example the form `id` is not specified and is therefore autogenerated by the library. While autogenerating form ids using the default export is convenient, **it is recommended though to explicitly assign forms their application-wide globally unique `id`s** if you plan on using advanced Redux features such as state saving and restoration otherwise form field values won't be restored upon restoring a previously saved Redux state. [Read more](https://github.com/halt-hammerzeit/simpler-redux-form#form-decorator-options). This is still an advanced feature and not everyone needs to know that.
...
This `@Form()` decorator creator takes the following options:
  ...
  * `id : String` — an application-wide globally unique form ID (because form data path inside Redux store is gonna be `state.form.${id}`). Alternatively this application-wide globally unique form id can be set via `formId` property passed to the decorated form component (say, if a form component is used on different pages of the app). If no `id` is set up for a form then it's autogenerated. **It is recommended though to name forms using explicit application-wide globally unique `id`s** because this way various advanced scenarios will work seamlessly: say, if Redux state is persisted to `localStorage` for offline work and then restored back when the page is opened again then the form fields won't loose their values if the form `id` is set explicitly because in case of an autogenerated one it will be autogenerated again and obviously won't match the old one (in the saved Redux state) so all form fields will be reset on a newly opened page.
