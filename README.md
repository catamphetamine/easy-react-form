...
Where `'orderForm'` argument is the explicitly set `id` of the form (inside Redux state tree) whereas in the former example the form `id` is not specifined and is therefore autogenerated by the library. While autogenerating form ids using the default export looks convenient, **it is recommended though to always explicitly assign forms their `id`s** if you plan on using advanced Redux features [such as state saving and restoration](https://github.com/halt-hammerzeit/simpler-redux-form#form-decorator-options) otherwise form field values won't be restored upon restoring a previously saved Redux state. This is still an advanced feature and not everyone needs to know that.
...
This `@Form()` decorator creator takes the following options:
  ...
  * `id : String` — an application-wide unique form ID (because form data path inside Redux store is gonna be `state.form.${id}`). Alternatively form id can be set via `formId` property passed to the decorated form component. If no `id` is set up for a form then it's autogenerated. **It is recommended though to name forms using explicit `id`s** because this way various advanced scenarios will work seamlessly: say, if Redux state is persisted to `localStorage` for offline work and then restored back when the page is opened again then the form fields won't loose their values if the form `id` is set explicitly because in case of an autogenerated one it will be autogenerated again and obviously won't match the old one (in the saved Redux state) so all form fields will be reset on a newly opened page.
